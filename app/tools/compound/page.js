...
export default function CompoundCalculator() {
  ...
  const [yilSayisi, setYilSayisi] = useState(5);

  const hesapla = () => {
    const r = faizOrani / 100;
    const finalAmount = anapara * Math.pow(1 + r, yilSayisi);
    const kazanc = finalAmount - anapara;
    return { finalAmount, kazanc };
  };

  const { finalAmount, kazanc } = hesapla();
    ...
    <div style={{minHeight: '100vh', backgroundColor: '#fff', fontFamily: 'system-ui, -apple-system, sans-serif'}}>
      <div style={{backgroundColor: '#f9f9f9', padding: '30px 20px', borderBottom: '1px solid #eee'}}>
        <div style={{maxWidth: '1200px', margin: '0 auto'}}>
          ...
          <h1 style={{fontSize: '36px', fontWeight: 'bold', color: '#000', margin: 0}}>Bileşik Faiz Hesaplama</h1>
        </div>
      </div>
      ...
      <div style={{maxWidth: '1200px', margin: '0 auto', padding: '40px 20px'}}>
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '40px', backgroundColor: '#f9f9f9', padding: '30px', borderRadius: '8px', border: '1px solid #eee'}}>
          ...
          <div>
            <label style={{display: 'block', fontWeight: 'bold', marginBottom: '8px', color: '#000', fontSize: '14px'}}>📅 Yıl</label>
            <input type="number" value={yilSayisi} onChange={(e) => setYilSayisi(parseInt(e.target.value))} style={{width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '6px', fontSize: '16px'}} />
          </div>
        </div>
        ...
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px'}}>
          ...
          <div style={{backgroundColor: '#f0fdf4', padding: '25px', borderRadius: '8px', border: '1px solid #dcfce7', textAlign: 'center'}}>
            ...
            <p style={{fontSize: '26px', fontWeight: 'bold', color: '#22c55e', margin: '8px 0 0 0'}}>+{kazanc.toLocaleString('tr-TR', {maximumFractionDigits: 2})} ₺</p>
          </div>
          <div style={{backgroundColor: '#dbeafe', padding: '25px', borderRadius: '8px', border: '1px solid #bfdbfe', textAlign: 'center'}}>
            ...
            <p style={{fontSize: '26px', fontWeight: 'bold', color: '#3b82f6', margin: '8px 0 0 0'}}>{finalAmount.toLocaleString('tr-TR', {maximumFractionDigits: 2})} ₺</p>
          </div>
        </div>
